...
Module       : AZURE AD
Date         : 2025-07-13
Objectif     : Exporter la configuration AZURE AD d'un utilisateur, le restaurer à son état d'origine,
               générer un rapport HTML, et notifier par Teams et par mail.
Script ID    : AZUREAD-EXTRACT-RESTORE-USER-v1.0
------------------------------------------------------------
...
# ========================================
# Script Extract_User.ps1 - Version Nico Enhanced
...
$TargetUserPrincipalName = "nico@M365x49418703.onmicrosoft.com"
$OutputPs1Path           = "C:\Script_complet\Scripts OK - PF Juin 2025\TEMPLATE_RESTO_OK\USERS\TESTUSER\RestoreNico\RestoreNico.ps1"
...
$LogDirectory = "C:\Script_complet\Scripts OK - PF Juin 2025\TEMPLATE_RESTO_OK\BACKUP\USERS\TESTUSER\log"
$LogFile = "$LogDirectory\UserRestore_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').log"
$ScriptStartTime = Get-Date

# === Configuration des logs HTML ===
$HtmlLogFile = "$LogDirectory\UserRestore_$(Get-Date -Format 'yyyy-MM-dd_HH-mm-ss').html"
$LogEntries = @()
...
<head>
    <meta charset="UTF-8">
    <title>Rapport de Restauration Utilisateur - $TargetUserPrincipalName</title>
    <style>
...
<body>
    <div class="header">
        <h1>🔄 Rapport de Restauration Utilisateur M365DSC</h1>
        <p><strong>Utilisateur:</strong> $TargetUserPrincipalName</p>
...
# -------------------------
# Fonction d'envoi Teams pour restauration utilisateur
# -------------------------
function Send-UserRestoreTeamsNotification {
    param (
        [string]$Message,
        [string]$Status,
        [string]$WebhookUrl = $TeamsWebhookUrl,
        [string]$UserPrincipalName = $TargetUserPrincipalName,
        [string]$Step = "",
    ...
    $Payload = @{
        ...
        "summary"    = "Restauration Utilisateur M365DSC"
        "sections"   = @(
            @{
                "activityTitle"    = "🔄 Restauration DSC - Utilisateur AzureAD/EntraID"
                "activitySubtitle" = "Statut : $Status"
                "activityImage"    = "https://raw.githubusercontent.com/Serge972/logosDSC365/main/Azure.png"
                "facts"            = @(
                    @{ "name" = "👤 Utilisateur"; "value" = $UserPrincipalName },
...
function Send-CriticalErrorNotification {
    ...
    Send-UserRestoreTeamsNotification -Message $ErrorMessage -Status "Failure" -WebhookUrl $CriticalChangesWebhookUrl -Step $Step -Details $Details
}
...
Write-LogMessage "DÉMARRAGE - Script Extract_User Enhanced" "INFO" "Startup"
Write-LogMessage "Utilisateur cible: $TargetUserPrincipalName" "INFO" "Startup"
Write-LogMessage "Fichier de configuration: $FilePS1_M365TenantConfig" "INFO" "Startup"
Write-LogMessage "========================================" "INFO" "Startup"

# Notification de démarrage
Send-UserRestoreTeamsNotification -Message "Démarrage de la restauration utilisateur" -Status "Info" -Step "🚀 Initialisation"
...
Send-UserRestoreTeamsNotification -Message "Fichier de configuration M365DSC trouvé et accessible" -Status "Success" -Step "1️⃣ Vérification fichier source"
...
$resourceName = "AADUser-$TargetUserPrincipalName"
Write-LogMessage "Recherche du bloc ressource: $resourceName" "INFO" "ContentAnalysis"

# === Étape 3 : Extraction du bloc AADUser ===
Write-LogMessage "ÉTAPE 3: Extraction du bloc AADUser" "INFO" "BlockExtraction"

# Recherche du bloc AADUser
$startIndex = $content.IndexOf("AADUser `"$resourceName`"")
if ($startIndex -lt 0) {
    $ErrorMsg = "Aucun bloc AADUser trouvé pour $TargetUserPrincipalName"
    Send-CriticalErrorNotification -ErrorMessage $ErrorMsg -Step "3️⃣ Extraction bloc AADUser"
    exit 1
}

Write-LogMessage "Bloc AADUser trouvé à l'index: $startIndex" "SUCCESS" "BlockExtraction"
...
# Construction du script RestoreNico.ps1
$scriptContent = @"
Configuration RestoreNico
{
$paramBlock
...
try {
    ...
    Send-UserRestoreTeamsNotification -Message "Script RestoreNico.ps1 généré avec succès" -Status "Success" -Step "6️⃣ Génération script" -Details $Details
}
catch {
...
try {
    . $OutputPs1Path
    Write-LogMessage "Script RestoreNico.ps1 chargé avec succès" "SUCCESS" "DSCCompilation"
    
    $compilationStart = Get-Date
    RestoreNico -ApplicationId $ApplicationId -CertificateThumbprint $CertificateThumbprint -TenantId $TenantId
    ...
    Send-UserRestoreTeamsNotification -Message "Compilation DSC terminée avec succès" -Status "Success" -Step "7️⃣ Compilation DSC" -Details $Details
}
catch {
...
try {
    ...
    Start-DscConfiguration -Path .\RestoreNico -Wait -Verbose -Force
    ...
    $Details = @{
        ...
        "📁 Chemin config" = ".\RestoreNico"
    }
    Send-UserRestoreTeamsNotification -Message "Configuration DSC appliquée avec succès" -Status "Success" -Step "8️⃣ Application DSC" -Details $Details
}
catch {
...
$FinalDetails = @{
    ...
    "👤 Utilisateur restauré" = $TargetUserPrincipalName
    "📋 Fichier log" = $LogFile
    "📊 Rapport HTML" = $HtmlLogFile
    "✅ Statut final" = "SUCCÈS COMPLET"
}

Send-UserRestoreTeamsNotification -Message "🎉 Restauration utilisateur terminée avec succès !" -Status "Success" -Step "✅ Finalisation" -Details $FinalDetails